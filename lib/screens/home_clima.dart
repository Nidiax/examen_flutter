  ...
  void cambiarTemperatura() {
    ...
    setState(() {
      if (temperatura <= 10 && (lluvia || !lluvia)) {
        estadoClima = 'Nevado';
        imagen = Icon(Icons.ac_unit, color: Colors.white, size: size,);
        return;
      }
      ...
      //rango <25
      if (temperatura >= 18 && viento >= 17 && viento < 25 && temperatura < 30 && lluvia==false) {
        estadoClima = 'Ventoso';
        imagen = Icon(CupertinoIcons.wind, color: Color.fromARGB(255, 63, 62, 62), size: size);
        return;
      }
      ...
      if (temperatura >= 18 && viento >= 17 && viento < 25 && lluvia && temperatura < 30) {
        estadoClima = 'Lluvioso';
        imagen = Icon(CupertinoIcons.cloud_drizzle, color: Colors.grey, size: size);
        return;
      }
      ...
      if(temperatura >= 18 && viento > 25 && lluvia && temperatura < 30){
        ...
        imagen = Icon(CupertinoIcons.cloud_bolt_rain, color: Colors.grey, size: size);
        return;
      }
      if(lluvia){
        estadoClima = 'Lluvioso';
